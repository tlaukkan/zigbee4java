...
public class StepCommand extends ZclCommand {
    ...
    public StepCommand(final ZclCommandMessage message) {
        super(message);
        this.stepMode = (Byte) message.getFields().get(ZclFieldType.STEP_COMMAND_STEP_MODE);
        this.stepSize = (Byte) message.getFields().get(ZclFieldType.STEP_COMMAND_STEP_SIZE);
        this.transitionTime = (Short) message.getFields().get(ZclFieldType.STEP_COMMAND_TRANSITION_TIME);
    }
    ...
    static {
        ZclUtil.registerCommandTypeClassMapping(ZclCommandType.STEP_COMMAND,StepCommand.class);
    }
}
