...
public class AddGroupResponseCommand extends ZclCommand {
    ...
    public AddGroupResponseCommand(final ZclCommandMessage message) {
        super(message);
        this.status = (Byte) message.getFields().get(ZclFieldType.ADD_GROUP_RESPONSE_COMMAND_STATUS);
        this.groupId = (Short) message.getFields().get(ZclFieldType.ADD_GROUP_RESPONSE_COMMAND_GROUP_ID);
    }
    ...
    static {
        ZclUtil.registerCommandTypeClassMapping(ZclCommandType.ADD_GROUP_RESPONSE_COMMAND,AddGroupResponseCommand.class);
    }
}
