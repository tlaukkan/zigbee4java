	...
	public static final String toHexString(long ieee){
		...
		for (int i = parts.length-1; i > 0; i--) {
			switch(parts[i].length()){
				case 1: buffer.append("0x0").append(parts[i]).append(' '); break;			
				case 2: buffer.append("0x").append(parts[i]).append(' ');  break;
			}
		}
		switch(parts[0].length()){
			case 1: buffer.append("0x0").append(parts[0]); break;			
			case 2: buffer.append("0x").append(parts[0]); break;
		}
